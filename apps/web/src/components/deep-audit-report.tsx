...
function humanizeIdentifier(value: string): string {
  const text = value.replace(/^deep-/, "").replace(/[-_]+/g, " ").trim();
...
function presentationFor(checkId: string): CheckPresentation {
  if (checkId === "deep-http-observation") {
    return {
      ...
      description: "Observe la réponse HTTP principale et son parcours de redirection.",
    };
  }
  if (checkId === "deep-browser-observation") {
    return {
      ...
      description: "Charge la page dans un navigateur isolé et borné pour observer son comportement réel.",
    };
  }
  ...
  const category =
    candidates.find(([tokens]) => tokens.some((token) => id.includes(token)))?.[1] ??
    "Autres contrôles";
  return {
    category,
...
const reasonLabels: Record<string, { title: string; detail: string }> = {
  ...
  "ssrf-denied": {
    ...
    detail: "Une destination non publique ou non autorisée a été refusée par les protections réseau.",
  },
  "budget-time": {
    title: "Temps d’analyse maximal atteint",
    detail: "Le budget de durée du Deep a été atteint avant la fin de toutes les observations.",
  },
  ...
  "budget-host-requests": {
    ...
    detail: "Le nombre maximal de requêtes autorisées vers un même hôte a été atteint.",
  },
...
function evidenceHighlights(run: DeepCheckRunView): string[] {
  ...
  for (const evidence of run.evidence) {
    ...
    if (typeof data.pageCount === "number") {
      highlights.push(
        data.pageCount + " page" + (data.pageCount > 1 ? "s" : "") + " observée" + (data.pageCount > 1 ? "s" : ""),
      );
    }
  }
...
export function DeepAuditReport({
  ...
  const reasons = Array.isArray(coverage?.reasons)
    ? coverage.reasons.filter((item): item is string => typeof item === "string")
    : [];
  const completed = checkRuns.filter((run) => run.status === "completed").length;
  const skipped = checkRuns.filter((run) => run.status === "skipped").length;
  const failed = checkRuns.filter((run) => run.status === "failed").length;

  const groups = checkRuns.reduce<Map<string, DeepCheckRunView[]>>((map, run) => {
    const category = presentationFor(run.checkId).category;
    map.set(category, [...(map.get(category) ?? []), run]);
    return map;
  }, new Map());

  const coverageComplete = status === "completed" && reasons.length === 0 && failed === 0;
  ...
  return (
    <section className="border-b border-[#242d40] py-10">
      ...
      <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h2 className="text-2xl font-semibold tracking-[-0.03em]">Synthèse Deep</h2>
        ...
        <span className="rounded-md border border-[#39445d] bg-[#111827] px-3 py-2 text-xs text-[#b6c0d1]">
          {coverageComplete ? "Couverture complète du périmètre" : "Couverture à vérifier"}
        </span>
      </div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div key={String(label)} className="am-panel-soft p-4">
            <p className="text-xs uppercase tracking-[0.12em] text-white/35">{String(label)}</p>
      ...
      {budget ? (
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={String(label)} className="rounded-lg border border-[#242d40] bg-[#0d111a] p-4">
      ...
      {reasons.length > 0 ? (
        <div className="mt-5 border-l-2 border-amber-300/70 bg-amber-200/[0.05] p-5">
          <p className="text-sm font-semibold text-amber-100">Certaines limites ont réduit la couverture</p>
          <div className="mt-3 space-y-3">
            {reasons.map((reason) => {
              const copy = reasonLabels[reason] ?? {
                title: "Limite technique rencontrée",
                detail: "Une limite interne a restreint une partie de l’analyse. Le détail technique reste disponible ci-dessous.",
              };
              return (
                <div key={reason}>
                  <p className="text-sm font-medium text-amber-100/90">{copy.title}</p>
                  <p className="mt-1 text-sm leading-6 text-amber-100/60">{copy.detail}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="mt-5 border-l-2 border-emerald-300/60 bg-emerald-200/[0.04] p-4">
          <p className="text-sm text-emerald-100/75">
            Tous les contrôles prévus ont été exécutés dans le périmètre et les budgets autorisés.
          </p>
        </div>
      ) : null}

      <div className="mt-8">
        <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#647188]">Ce qui a été analysé</p>
                      ...
                      <article key={run.id} className="rounded-lg border border-[#242d40] bg-[#0d111a] p-5">
                        ...
                        {highlights.length > 0 ? (
                          <div className="mt-4 flex flex-wrap gap-2">
                            {highlights.map((item) => (
                              <span key={item} className="rounded-md border border-[#303a50] bg-[#111827] px-2.5 py-1 text-xs text-[#aeb9cc]">
                                {item}
                              </span>
                            ))}
                          </div>
                        ...
                        <details className="mt-5 border-t border-[#242d40] pt-4">
                          <summary className="cursor-pointer text-xs font-semibold text-[#8793ff]">Détails techniques</summary>
                          <dl className="mt-4 grid gap-3 sm:grid-cols-2">
                            <div>
                              <dt className="text-xs text-white/30">Identifiant interne</dt>
                              <dd className="mt-1 break-all font-mono text-xs text-white/55">{run.checkId} · v{run.checkVersion}</dd>
                            </div>
                            <div>
                              <dt className="text-xs text-white/30">Durée</dt>
                              <dd className="mt-1 text-xs text-white/55">{run.durationMs === null ? "—" : run.durationMs + " ms"}</dd>
                            </div>
                          </dl>
                          {Object.keys(run.budgetUsed).length > 0 ? (
                            <pre className="mt-3 overflow-x-auto rounded-md bg-black/20 p-3 text-xs leading-5 text-white/55">{JSON.stringify(run.budgetUsed, null, 2)}</pre>
                          ) : null}
                          {run.evidence.length > 0 ? (
                            <pre className="mt-3 overflow-x-auto rounded-md bg-black/20 p-3 text-xs leading-5 text-white/55">{JSON.stringify(run.evidence, null, 2)}</pre>
                          ) : null}
                        </details>
                      </article>
                    );
