...
Deep Audit is still rejected by dispatch, scan-context validation, and the worker processor. Before enabling it, connect the candidate browser and HTTP collectors to the complete scan lifecycle, add grant issuance and revocation workflows, verify browser interception against a real Chromium instance, and run the new migration and PostgreSQL integration tests on Linux. Active-safe checks have no network facade yet and remain unavailable. V2 keeps its existing route and proxy behavior.
